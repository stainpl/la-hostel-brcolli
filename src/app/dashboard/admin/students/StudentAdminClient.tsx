...
export default function StudentsAdminClient({ students, page, totalPages, currentGender, currentYear }: Props) {
  ...
  const [actionInfo, setActionInfo]     = useState<{ id: number; type: 'clear' | 'delete' } | null>(null)
    ...
    <>
          ...
          <thead className="bg-gray-50">
            <tr>
              {['Name', 'Email', 'Room', 'Gender', 'Year', 'Actions'].map(header => (
                <th key={header} className="px-4 py-2 text-xs font-medium text-gray-500 uppercase">
                  {header}
                </th>
              ))}
            </tr>
            ...
            {students.map(s => (
              <tr key={s.id}>
                ...
                <td className="px-4 py-2 text-sm flex space-x-2">
                  <Button size="sm" variant="outline" onClick={() => router.push(`/dashboard/admin/students/${s.id}/edit`)}>
                    Edit
                  </Button>
      ...
      {totalPages > 1 && (
        <div className="flex items-center space-x-2 mt-4">
          <Button variant="secondary" size="sm" onClick={() => goToPage(page - 1)} disabled={page === 1 || isPending}>
            ← Prev
          </Button>
          <span>Page {page} of {totalPages}</span>
          <Button variant="secondary" size="sm" onClick={() => goToPage(page + 1)} disabled={page === totalPages || isPending}>
            Next →
          </Button>
        </div>
      )}
