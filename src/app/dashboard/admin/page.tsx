...
export default function AdminDashboard() {
  ...
  // Fetch export data when modal opens or filters change
  useEffect(() => {
    ...
    fetch(`/api/admin/students?${params.toString()}`)
      ...
      .then((data) => {
        ...
        if (Array.isArray(data?.students)) {
          setExportData(data.students as Student[])
        } else {
          setExportData([])
        }
      })
  ...
  const handleFreshSession = async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        ...
        setSessionMsg('Failed to start session.')
      }
    } finally {
      setIsLoading(false)
    }
        ...
        <ConfirmModal
          ...
          description={
            <div className="space-y-2">
              <p>
                You&apos;re about to clear every student&apos;s room assignment and reset payment flags. Enter
                the new session label below:
              </p>
              <input
                type="text"
                value={sessionInput}
          ...
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-2xl space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  className="form-select w-full text-green-600 border-gray-300 focus:border-green-500 focus:ring focus:ring-opacity-50"
                  value={selectedYear}
